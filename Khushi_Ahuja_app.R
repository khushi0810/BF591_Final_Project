...
ui <- fluidPage(
  ...
  ")),
  titlePanel("BF 591 Final Project: Khushi Ahuja"),
  tabsetPanel(
    type = "tabs",
    tabPanel("Sample Exploration",
             p("This tab allows the user to input metadata csv file and explore the sample information"),
             sidebarLayout(
               sidebarPanel(
                 fileInput("Sample_file", "Choose a Sample Information File (ony CSV File)", accept = ".csv", placeholder = "sample information file"),
                 submitButton("Submit"), width = 3
               ),
               mainPanel(
                 tabsetPanel(
                   type = "tabs",
                   tabPanel("Summary", p("This subtab provides the statstics summary ") , tableOutput("summary_table")),
                   tabPanel("Table", p("This subtab provides the sample information table which is sortable"), DT::dataTableOutput("sample_table")),
                   tabPanel("Plots", p("This tab provides a histogram plot for choosen variable"),
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                colourInput("samp_color_1", "Outline color","#184D9E"),
                                colourInput("samp_color_2", "Fill color color","#2EA32A"),
                                submitButton("Submit")
                              ),
    ...
    tabPanel("Counts Exploration",
             p("This tab allows the user to input metadata csv file and explore the data"),
             sidebarLayout(
               sidebarPanel(
                 fileInput("Normalized_counts_matrix", "Choose a Normalized Counts Matrix (ony CSV File)", accept = ".csv", placeholder = 'normalized counts matrix file'),
                 sliderInput("var_slider", "Percentile of Variance", min = 0, max = 100, value = 40),
                 sliderInput("zero_slider", "Number of Samples that are non-zero", min = 0, max = 100, value = 40),
                 submitButton("Submit"), width = 3
               ),
               mainPanel(
                 tabsetPanel(
                   type = "tabs",
                   tabPanel("Table",
                            p("This subtab provides the statistics based on the input parameters(percentile of variance and number of samples that are non-zero)"),
                   ...
                   tabPanel("Heat map",
                            p("This subtab provides a heat map of the normalized counts input matrix"), 
                            plotOutput("heatmap")),
                   tabPanel("pca", selectInput(inputId = "comp1", label="Select X-axis", choices = c("PC1", "PC2", "PC3", "PC4", "PC5", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10")),
                            ...
                            p("This subtab provides a pca plot"),
    ...
    tabPanel("Differential Expression",
             p("This tab is about exploring the results of differential expression on uploading a Differntial analysis File"),
             sidebarLayout(
               sidebarPanel(
               ...
               mainPanel(
                 tabsetPanel(
                   type = "tabs",
                   tabPanel("Table",
                            p("This subtab provides sortable table of the differential expression analysis results."),
                            DT::dataTableOutput("table",width = "80%")),
                   tabPanel("Volcano Plot", 
                            p("This subtab provides a volcano plot based on the input paramter (X-axis,Y-axis and padjusted)"),
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                radioButtons("y_axis", "Y-Axis",
                                             choices = c("log2FoldChange" = "log2FoldChange","baseMean" = "baseMean", "lfcSE"="lfcSE","stat"="stat","pvalue"="pvalue","padj"="padj"),
                                             selected = "padj"),
                                sliderInput(inputId = "padjusted", min = -35, max = 0,label = "Select the magnitude of the p adjusted ", value = -15, step = 1),
                                colourInput("de_base", "Base point color","#D61515"),
                                colourInput("de_highlight", "Highlight point color","#2A42A1"),
                                submitButton("Submit"),width=3
                              ),
                              mainPanel(
                                plotOutput("volcano"))))
    ...
    tabPanel("Individual Gene Expression",
             ...
             sidebarLayout(
               sidebarPanel(
                 fileInput(inputId = 'IGE_countsID', label = 'Choose a Normalized Counts Matrix File (Only CSV File)',accept = ".csv", placeholder = "normalized counts matrix file"),
                 fileInput(inputId = 'IGE_metaID', label = 'Choose a Sample Information CSV File (Only csv File)',accept = ".csv", placeholder = "sample information file"),
                 selectInput("metachoice", choices = c("Sample_diagnosis"="Sample_diagnosis", "Sample_pmi"="Sample_pmi","Sample_Age_of_Death"="Sample_Age_of_Death", "Sample_rin"="Sample_rin", "Sample_mrna_seq"="Sample_mrna_seq"),
                             label = "Select Category Field", selected = "Sample_diagnosis"),
                 #insert gene search box here
                 textInput("gene", label = "Enter Ensembl ID of gene (For ex: ENSG00000000003.10)"),
                 radioButtons("type_plot", "Choose the Plot Type",
